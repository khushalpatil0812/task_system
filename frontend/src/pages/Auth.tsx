...
function AuthForm({ signup = false }: { signup?: boolean }) {
  ...
  const submit = async (values: Record<string, string>) => {
    try { signup ? await join(values) : await login(values.email, values.password); toast.success(signup ? 'Account created' : 'Welcome back'); navigate('/'); }
    catch (error: any) { toast.error(error.response?.data?.detail || 'Something went wrong'); }
  };
...
export const Signup = () => <AuthForm signup />;
